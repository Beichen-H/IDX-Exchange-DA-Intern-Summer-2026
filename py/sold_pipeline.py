"""Incrementally synchronize closed CRMLS sales into ../csv/sold.csv."""
...
# Existing CRMLS endpoints retained from crmls_sold.py.
...
SELECT_FIELDS = (
    ...
    "UnparsedAddress,PropertyType,LivingArea,ListPrice,DaysOnMarket,ListOfficeName,"
    ...
    "ListingId,BathroomsTotalInteger,City,TaxYear,BuildingAreaTotal,BedroomsTotal,"
...
CSV_PATH = (Path(__file__).resolve().parent / ".." / "csv" / "sold.csv").resolve()
...
def build_filter(start: datetime, end: datetime) -> str:
    return (
        f"MlsStatus eq 'Closed' and {DATE_COLUMN} ge {format_odata_timestamp(start)} "
        f"and {DATE_COLUMN} lt {format_odata_timestamp(end)}"
    )
...
def run_pipeline(
    ...
    """Run one complete sold-property synchronization and return the saved row count."""
    csv_path = Path(csv_path)
    ...
    if start > end:
        raise ValueError("Local maximum CloseDate is later than current UTC time")

    print(
        f"Sold window: {format_odata_timestamp(start)} <= {DATE_COLUMN} < "
    ...
    if not records and existing_file:
        print("No new sold records; existing CSV was left unchanged.", flush=True)
        return len(existing)
    ...
    print(
        f"Sold records saved to {csv_path}: {len(merged)} rows; "
...
def main() -> int:
    try:
        ...
        print(f"Sold pipeline failed: {exc}", flush=True)
        return 1
